  ...
  const tooltipHooks = useMemo(
    () =>
      buildChartTooltipHooks({
        ...
        setTooltip,
        buildRows: (index) => [
          { label: "上行", value: formatByteRateLabel(Number(dataRef.current[1]?.[index] ?? 0)), color: UP_COLOR },
          { label: "下行", value: formatByteRateLabel(Number(dataRef.current[2]?.[index] ?? 0)), color: DOWN_COLOR },
        ],
      }),
    [],
  );
  ...
  const baseOptions = useMemo<Omit<uPlot.Options, "width" | "height">>(() => {
    ...
    return {
      ...
      axes: [
        ...
        { stroke: text, grid: { stroke: grid, width: 1 }, ticks: { stroke: grid }, size: compact ? 70 : 82, values: (_self, splits) => splits.map(axisRate) },
      ],
      series: [
        { label: "时间" },
        { label: "上行", stroke: UP_COLOR, fill: `${UP_COLOR}12`, width: 1.8, points: { show: false } },
        { label: "下行", stroke: DOWN_COLOR, width: 1.8, points: { show: false } },
      ],
      hooks: {
        init: [
          (plot) => {
