...
const STORAGE_KEY = "cfsm-nier:renewal-reminders:v1";
...
function AssetLink() {
  return (
    <Link to="/assets" className="overview-card-action" aria-label="打开资产统计页" title="资产统计">
...
export function RenewalReminder({ nodes }: { nodes: RenewalReminderSource[] }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => setClock(Date.now()), Math.max(1_000, nextWakeAt - now + 50));
  ...
  return (
    <div className="renewal-reminder" ref={rootRef}>
      ...
      {open && (
        <section id={panelId} className="renewal-reminder-panel" role="dialog" aria-labelledby={titleId}>
          <header className="renewal-reminder-head">
            ...
            <button ref={closeRef} type="button" className="renewal-reminder-close" aria-label="关闭续费提醒" title="关闭" onClick={closeAndRestoreFocus}>
              <span aria-hidden>✕</span>
            </button>
          </header>
          ...
          <div className="renewal-reminder-list">
            ...
            {hiddenCount > 0 && <p className="renewal-reminder-more">还有 {hiddenCount} 台临期节点</p>}
          </div>
          ...
          <footer className="renewal-reminder-actions">
            ...
            <button type="button" className="renewal-reminder-dismiss" onClick={dismissCurrentCycles}>
              本周期不再提醒
            </button>
          </footer>
        </section>
