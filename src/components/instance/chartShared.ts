...
// 两者都取自这一处单一来源。NieR 版收成哑光大地色系，保持可分辨。
...
// uPlot 图表的坐标轴网格/文字颜色。NieR 版用炭黑/米白。
export function getAxisColors(isDark: boolean): { grid: string; text: string } {
  return {
    grid: isDark ? "rgba(216,209,187,0.09)" : "rgba(26,24,20,0.10)",
    text: isDark ? "#8f8871" : "#57523f",
  };
}
